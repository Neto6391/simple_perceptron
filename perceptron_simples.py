...
for i in range(len(conjunto_amostras)):
	conjunto_amostras[i].insert(0,-1)

print("Carregando %d amostras de dados\nCom %d dados de saída\n" %(len(conjunto_amostras), len(conjunto_saida)))
print(conjunto_amostras)
	
w = [random() for i in range(len(conjunto_amostras[0]))]
w.insert(0, -1)
print("Peso ", w)
num_epoca = 0
epoca = 1000
...
erro = True
while num_epoca < epoca or not erro:
	
	for i in range(len(conjunto_amostras)):
		
		u = 0
		
		for j in range(len(conjunto_amostras[0])):
			u += w[j] * conjunto_amostras[i][j]
		...
		if y != conjunto_saida[i]:
			...
			for j in range(len(conjunto_amostras[0])):
				...
				w[j] = w[j] + taxa_aprendizado * erro_aux * conjunto_amostras[i][j]
				erro = True
		print(num_epoca)
		num_epoca += 1